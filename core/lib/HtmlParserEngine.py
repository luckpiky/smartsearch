...
import urllib
import string
from core.lib.HTMLParser2 import *
...
########################################################
def searchInfo(rule, url, code, func):
    #compile rule
    rule_content = open(rule).read().decode(code)
    cmpl = HtmlParserCompiler()
    cmpl.feed(rule_content)

    #get info
    page = urllib.urlopen(url).read().decode(code)
    pgcmpl = HtmlParserMatch(cmpl.rule)
    pgcmpl.set_initfunc(func)
    pgcmpl.feed(page)
